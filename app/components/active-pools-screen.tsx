...
export function ActivePoolsScreen({ pools, userName }: { pools: FeedPool[], userName?: string | null }) {
  ...
  if (pools.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-300 bg-zinc-50 py-12 text-center">
        ...
        <h3 className="text-sm font-semibold text-zinc-900">No active pools right now</h3>
        <p className="mt-1 text-xs text-zinc-500">Be the first to raise a pool and find co-riders!</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-bold text-zinc-900">Available Pools</h1>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 border-t border-zinc-100 pt-3">
              {(() => {
                  ...
                  return (
                    <div className="flex w-full flex-col gap-2">
                      <div className="flex w-full gap-2">
                        <button 
                          disabled={isPending}
                          onClick={() => startTransition(async () => { await completePool(pool.id); router.refresh() })}
                        ...
                        <button
                          disabled={isPending}
                          onClick={() => startTransition(async () => { await deletePool(pool.id); router.refresh() })}
                ...
                if (hasJoined) {
                  return (
                    <div className="flex w-full flex-col gap-2">
                      <button type="button" disabled className="w-full rounded-md bg-zinc-200 px-3 py-2 text-[13px] font-medium text-zinc-500">
                        Joined!
                      </button>
                      <ChatBox poolId={pool.id} userName={activeUser} />
                    </div>
                ...
                return (
                  <button
                    type="button"
                    disabled={isPending}
                    onClick={() => startTransition(async () => { await joinPool(pool.id); router.refresh() })}
